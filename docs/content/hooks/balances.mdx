...
Passing `null`/`undefined` as the address keeps the hook idle (no request) — convenient before a wallet exists. For raw-value formatting, see the `formatSats` / `formatBtc` / `satsToBtc` / `btcToSats` utilities exported from `@sbtc/sdk`.
