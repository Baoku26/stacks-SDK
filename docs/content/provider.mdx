...
```tsx
import { useSbtcContext } from '@sbtc/sdk';
...
On **native**, if `SbtcProvider` mounts before `@sbtc/sdk/polyfills` was imported (so `global.Buffer` is missing), it throws [`POLYFILL_NOT_INITIALIZED`](/security#errors). Import the polyfills as the first line of your entry file — see [Polyfills](/polyfills).
