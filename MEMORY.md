...
**CI (T005):** `.github/workflows/ci.yml` (push to main + PRs). `pnpm/action-setup@v4` takes the pnpm version from `packageManager`; Node 20; pnpm cache. `publish.yml` triggers on `v*` tags → build → `pnpm --filter @sbtc/sdk publish --access public --provenance` using `secrets.NPM_TOKEN`.

**CI is scoped to `@sbtc/sdk` only** — install is `pnpm install --frozen-lockfile --filter @sbtc/sdk`, and each check runs via `pnpm exec turbo run <task> --filter=@sbtc/sdk`. WHY: the first CI run (a full-workspace install + root `pnpm lint`) failed on the Linux runner while everything passed locally on macOS/arm64. Root cause was not reproducible locally (couldn't read the runner log — `gh` absent, logs API needs admin auth), but the prime suspect is the example apps' platform-native binaries (Next SWC, `sharp`, `unrs-resolver`) interacting badly with a darwin-generated lockfile on `--frozen-lockfile`. The SDK (`packages/core`) is pure TS and won't fail on Linux. Scoping CI to the package it is supposed to gate is both the fix and the correct design — the Expo/Next example apps are M8 integration references, not part of the M1 SDK gate. If a future CI failure needs the exact log, install `gh` or fetch with an admin token. Verified locally: the scoped install + the four turbo-filtered checks all exit 0; `test:coverage` no-ops until vitest lands (M2+).
